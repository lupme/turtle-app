...
def get_tcr_score(stock_code: str, current_price: int) -> dict:
    if not stock_code or current_price <= 0:
        return {"score": 0, "status": "데이터 부족", "color": "rgb(108,122,137)"}
    ...
    if tcr_score >= 75: col, stt = "rgb(70,130,180)", f"강력 기동 [{detail}]"
    elif tcr_score <= 40: col, stt = "#e06666", f"OCP 작동 [{detail}]"
    else: col, stt = "rgb(108,122,137)", f"추세 관찰 [{detail}]"
...
def get_analysis_legend() -> str:
    return """<div style="margin-top: 10px; padding: 15px; border-top: 1px solid #1e293b; border-radius: 8px;">
        <p style="color: rgb(108,122,137); font-size: 0.8rem; font-weight: 700; margin-bottom: 5px;">[M01: T-Q Engine Baseline V49.3]</p>
        <p style="color: rgb(108,122,137); font-size: 0.75rem; margin: 0;">* 외국인수급(40%) + 52주추세(40%) + VCP방어력(20%)</p></div>"""
