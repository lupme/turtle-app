...
def get_tcr_score(stock_code: str, current_price: int, weights: dict = None) -> dict:
    ...
    return {"score": tcr_score, "status": stt, "color": col, "f": f_s, "t": t_s, "v": v_s}

# 🚨 병렬 처리(Thread)로 속도를 3배 이상 끌어올리는 일괄 수집 엔진
def get_tcr_scores_batch(stock_list: list, weights: dict = None) -> dict:
    results = {}
    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
        future_to_code = {
            executor.submit(get_tcr_score, code, price, weights): code
...
def get_analysis_legend(weights: dict = None) -> str:
    ...
    return f"""<div style="margin-top: 10px; padding: 15px; border-top: 1px solid #1e293b; border-radius: 8px;">
        <p style="color: #6C7A89; font-size: 0.8rem; font-weight: 700; margin-bottom: 5px;">[M01: T-Q Engine V0.4.2 - Deep Analysis]</p>
