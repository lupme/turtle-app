...
# --- 오리지널 설정 ---
st.set_page_config(page_title="거북이 함대 기동 본부 V0.4.1 (AI 이식판)", layout="wide", initial_sidebar_state="expanded")
...
try:
    ...
    st.markdown('<div class="hq-title">🐢 TURTLE COMMAND HQ V0.4.1 (AI 이식판)</div>', unsafe_allow_html=True)
    ...
    with tab3:
        ...
        if st.button("🔥 작전 지시서 생성", use_container_width=True):
            if not api_key_input: 
                st.error("사이드바 하단에 API Key를 확인하십시오.")
            else:
