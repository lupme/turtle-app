...
st.markdown("""
    ...
    .index-name { font-size: 0.75rem; color: rgb(108,122,137); font-weight: 500; margin-bottom: 2px; }
    ...
    .kpi-label { font-size: 0.8rem; color: rgb(108,122,137); font-weight: 500; margin-bottom: 4px; }
    .kpi-val { font-size: 1.4rem; font-weight: 600; letter-spacing: -0.5px; }
    .kpi-delta { font-size: 0.85rem; font-weight: 500; margin-left: 6px; }
    
    .text-blue { color: rgb(70,130,180) !important; }
    .text-red { color: #fca5a5 !important; } /* 눈이 편안한 소프트 레드 */
    ...
    .stock-name { font-size: 0.95rem; font-weight: 500; color: #ffffff; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
    ...
    .val-num { font-size: 0.95rem; font-weight: 500; }
...
def generate_ai_briefing(api_key, portfolio_df, tcr_results, indices, user_context):
    try:
        ...
        for _, r in portfolio_df.iterrows():
            ...
            pf_summary.append(f"- {r['종목명']}: 수익률 {r['안전_수익률']:.2f}%, TCR확신율 {tcr}점")
        ...
        # 2. 구글 본사에 현재 키로 접근 가능한 모델 목록 요청 (404 원천 차단)
        available_models = [m.name for m in genai.list_models() if 'generateContent' in m.supported_generation_methods]
        
        if not available_models:
            return "🚨 통신 실패: 해당 API 키는 텍스트 생성 모델에 접근할 권한이 없습니다. 구글 클라우드 콘솔을 확인해 주십시오."
            
        # 3. 승인된 모델 중 가장 좋은 모델(flash 우선) 자동 선택
        ...
        return f"🚨 통신 시스템 에러: {str(e)}\n\n사령관님, 구글 API 키의 결제 계정 연동이나 권한 문제가 확실합니다."

# --- Main App ---
try:
    sheet, df, full_df = load_data()
    indices = get_market_indices()
    st.markdown('<div class="hq-title">🐢 TURTLE COMMAND HQ V0.5.4 (Mobile & SDK)</div>', unsafe_allow_html=True)
    
    with st.sidebar:
        st.header("🎯 전략 사령부")
        
        st.markdown("**🤖 AI 참모 통신 채널**")
        # 세션에 저장된 키를 불러와 입력창에 자동 채움
        input_key = st.text_input("Gemini API Key", value=st.session_state["gemini_api_key"], type="password")
        if input_key != st.session_state["gemini_api_key"]:
            st.session_state["gemini_api_key"] = input_key # 새로운 키 입력 시 세션 업데이트
    ...
    tab_main, tab_analysis, tab_ai = st.tabs(["🚀 기동 관제실", "🔬 전술 분석실", "🤖 AI 브리핑"])
    ...
    with tab_main:
        ...
        for _, row in list_df.iterrows():
            ...
            # [오류 해결] 100이 중복 곱해지던 문제 완전 차단
            ys = f"<span class='{y_cl}'>{y_tri}</span><span class='text-white'>{abs(y_val):.2f}%</span>"
            ...
            <details class="premium-card">
                ...
                <div class="card-body">
                    <div class="metric-grid" style="display:grid; grid-template-columns:repeat(2,1fr); gap:12px; background:#020617; padding:15px; border-radius:0 0 10px 10px; border-top:1px solid #1e293b;">
                        <div class="metric-box"><div class="metric-label" style="font-size:0.75rem; color:rgb(108,122,137);">평가 금액</div><div class="metric-value" style="font-size:0.95rem; font-weight:600; color:rgb(70,130,180);">{row['평가금액']:,.0f}</div></div>
                        <div class="metric-box"><div class="metric-label" style="font-size:0.75rem; color:rgb(108,122,137);">🔥 확신율 (TCR)</div><div class="metric-value" style="font-size:0.95rem; font-weight:600; color:{tcr_info.get('color', 'text-gray')};">{tcr_info.get('score', 0)}%</div></div>
                        <div class="metric-box"><div class="metric-label" style="font-size:0.75rem; color:rgb(108,122,137);">매수 금액</div><div class="metric-value" style="font-size:0.95rem; font-weight:600; color:#ffffff;">{row['매수금액']:,.0f}</div></div>
                        <div class="metric-box"><div class="metric-label" style="font-size:0.75rem; color:rgb(108,122,137);">보유 수량</div><div class="metric-value" style="font-size:0.95rem; font-weight:600; color:#ffffff;">{row['잔고수량']:,.0f}</div></div>
                    </div>
                </div>
            </details>"""
        st.markdown(html_cards, unsafe_allow_html=True)

    # --- 📝 전술 분석실 (서술형 요약 메인 + 그래프는 하단으로 숨김) ---
    with tab_analysis:
        st.subheader("📝 AI 전술 요약 (서술형)")
        st.markdown("<p style='font-size:0.85rem; color:#94a3b8;'>모바일 가독성을 위해 현재 함대 종목들을 4가지 작전 상태로 분류했습니다.</p>", unsafe_allow_html=True)
        ...
        with st.expander("📊 시각화 데이터 (AI 전술 사분면 차트) 열기"):
            ...
            plot_df['표시수익률'] = plot_df['안전_수익률']
            
            base_chart = alt.Chart(plot_df).mark_circle(size=120).encode(
                x=alt.X('TCR점수:Q', scale=alt.Scale(domain=[0, 100]), title='TCR 확신율'),
                y=alt.Y('표시수익률:Q', title='현재 수익률 (%)'),
                color=alt.Color('TCR점수:Q', scale=alt.Scale(range=['rgb(70,130,180)', '#fca5a5'], domain=[0, 100]), legend=None),
    ...
    with tab_ai:
        ...
        if st.button("🔥 작전 지시서 생성", use_container_width=True):
            if not st.session_state["gemini_api_key"]:
                ...
                with st.spinner("🧠 구글 공식 SDK(직통선)를 통해 모델 권한 확인 중..."):
