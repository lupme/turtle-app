...
def get_gspread_client():
    # Streamlit Secrets에서 보안 키를 안전하게 인출
    key_info = json.loads(st.secrets["google_credentials"])
    # 팁: Invalid JWT 오류 방지를 위해 private_key의 줄바꿈 문자를 정규화합니다.
...
def load_data():
    ...
    # 데이터 무결성: 모든 데이터를 읽어오되 숫자형 변환을 시도합니다.
    full_df = pd.DataFrame(sheet.get_all_records())
    
    # 사령관님이 지정하신 핵심 9개 열만 정밀 추출
...
# --- [2] 전술 디자인 수칙 (Steel Blue / Slate Gray) ---
def apply_commander_design(styled_df):
    ...
    styled_df = styled_df.map(color_logic) # 최신 map 규격 준수
    
    # 핵심 지표 Bold 처리
    bold_cols = ['종목명', 'PPID', 'UPEH']
    available_bold = [c for c in bold_cols if c in styled_df.columns]
    if available_bold:
        styled_df = styled_df.set_properties(**{'font-weight': 'black'}, subset=available_bold)
...
try:
    ...
    with st.sidebar:
        ...
        with st.form("trade_form"):
            ...
            sel_stock = st.selectbox("종목 선택", stock_list if stock_list else ["선택 불가"])
